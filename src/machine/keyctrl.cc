...
Keyboard_Controller::Keyboard_Controller () : ctrl_port (0x64), data_port (0x60) {
  ...
  while(true){
	...
	if(status&outb){//0x01 wenn Ausgabepuffer voll
		data_port.inb();//leeren
		continue;
	}
	if(!(status&inpb))//0x02 wenn Eingabepuffer leer
		break;
  }
}
...
void Keyboard_Controller::reboot () {
  ...
  *(unsigned short*) 0x472 = 0x1234;
  ...
  } while ((status & inpb) != 0);//solange Eingabepuffer nicht leer
...
Key Keyboard_Controller::key_hit () {
  ...
  while (!(ctrl_port.inb() & outb)){}//warten bis antwort da
  code = data_port.inb();
...
void Keyboard_Controller::set_repeat_rate (unsigned char speed, unsigned char delay) {
  while (ctrl_port.inb() & inpb);//Eingabepuffer leer

  data_port.outb(cmd_set_speed);//0x60(0xf3)

  while (ctrl_port.inb() & inpb 
    || !(ctrl_port.inb() & outb));//Antwort da 
...
void Keyboard_Controller::set_led (Leds led, bool on) {
  ...
  data_port.outb(cmd_set_led);//0x60(0xed)
